...
namespace Tester1
{
    public class Gist
    {
        ...
        public double[,] gistGabor(double[,] imgsrc, Size newsize, Parameter param, ref GistContainer gistcontainer, int numberOfImage)
        {
            ...
            double[,] g = new double[W * Nfilters,N];
            ...
            if (numberOfImage==1)
            {
                ...
                gistcontainer.refImage = new double[gistcontainer.refImageRows, gistcontainer.refImageCols];
            ...
            else 
            {
                ...
                gistcontainer.secondImage = new double[gistcontainer.secondImageRows, gistcontainer.secondImageCols];
        ...
        public static void Main()
        {
            ...
            foreach (string path in files)
            {
                if (File.Exists(path))
                {
                    // This path is a file
                    var watch = Stopwatch.StartNew();
                    gist.LMgist(path, ref gistcontainer, fileindex++);
                    watch.Stop();
                    var elapsedSec = watch.ElapsedMilliseconds / 1000.0;
                    using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"D:/Codes/Petrel 2015 Plugin/LMgistPlugin1/Tester1/Resources/result.txt", true))
                    {
                        file.WriteLine("Distance1-{2} : {0} elapsed time: {1}", gist.FindDistance(gistcontainer), elapsedSec, path);
                    }
                }
                //else if (Directory.Exists(path))
                //{
                //    // This path is a directory
                //    gist.ProcessDirectory(path, gist, ref gistcontainer, fileindex++);
            ...
            string[] subdirectoryEntries = Directory.GetDirectories(fbd.SelectedPath);
            foreach (string subdirectory in subdirectoryEntries)
                gist.ProcessDirectory(subdirectory, gist, ref gistcontainer, fileindex++);
        ...
        public void ProcessDirectory(string targetDirectory, Gist gist, ref GistContainer gistcontainer, int fileindex)
        {
