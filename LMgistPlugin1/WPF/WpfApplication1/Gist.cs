...
namespace Tester1
{
    public class Gist
    {
        ...
        public GistData refImgGistData;
        ...
        public void LMgist(string filename, bool isRefImag = false) 
        {
            try 
            {
                ...
                JsonWrite(filename, gist,isRefImag);

                Console.Write("Done writing gist in Json\n");
                //return gist;
        ...
        public void JsonWrite(string filename, double[] gist, bool isRefImg)
        {
            ...
            string onlyfilename = filename.Replace(folder, "");
            if (isRefImg == false)
            {
                var json = JsonConvert.SerializeObject(gistdata, Formatting.Indented);
                System.IO.File.WriteAllText(@defaultOutputDirectory + onlyfilename, json);
            }
            else
                this.refImgGistData = gistdata;
        }
        public GistData JsonRead(string filename)
        {
            JObject jobj = JObject.Parse(File.ReadAllText(@filename));
            GistData gistdata = new GistData();
            gistdata.filename = (string)jobj["filename"];
            JArray a = (JArray)jobj["gistvalue"];
            //List<double> gistvalues = a.ToObject<List<double>>();
            gistdata.gistvalue = a.ToObject<List<double>>();
            try
            {
                gistdata.metadata = (string)jobj["metadata"];
        ...
        public double FindDistance(GistData refgistdata, GistData gistdata)
        {
            double sum = 0;
            for (int i = 0; i < refgistdata.gistvalue.Count; i++)
            {
                sum = sum + Math.Pow((refgistdata.gistvalue[i] - gistdata.gistvalue[i]), 2);
            }
            return sum;
        }
