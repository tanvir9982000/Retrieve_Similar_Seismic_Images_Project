...
namespace LMgistPlugin1
{
    public class Gist
    {
        ...
        public Bitmap GetBitMapGreyMatrix(string bitmapFilePath)
        {
        ...
        public double[][] prefilt(double[][] img_grey, Size newsize, int fc = 4)
        {
            ...
            double[][] prefiltOut = new double[newsize.Height][];
            for (int i = 0; i < newsize.Height; i++)
            {
                prefiltOut[i] = new double[newsize.Width];
            }
            ...
            //Print2DArray(prefiltOut, newsize.Height, newsize.Width);
        ...
        public double[][] imresizecrop(Bitmap img, int paramHeight, int paramWidth)
        {
            ...
            double[][] imgOut = new double[paramHeight][];
            for (int i = 0; i < paramHeight; i++)
                imgOut[i] = new double[paramWidth];
        ...
        public void LMgist(string filename, ref GistContainer gistcontainer, int numberOfImage) 
        {
            try 
            {
                Bitmap D = GetBitMapGreyMatrix(filename);
                //PetrelLogger.InfoBox("Bitmap Image size: " + this.bitmapImageWidth + "x" + this.bitmapImageHeight + " " + D[0][0].A + " " + D[0][0].R + " " + D[0][0].G + " " + D[0][0].B);
                
                if (param.ImageSize[0] == 0 || param.ImageSize[1] == 0)
                {
                    this.Param.ImageSize[0] = bitmapImageWidth;
                    this.Param.ImageSize[1] = bitmapImageHeight;
                }

                param.G = createGabor(Param.OrientationsPerScale, param.ImageSize[0] + 2 * Param.BoundaryExtension);
                ...
                double[][] gist = new double[Nscenes][];
                for(int i=0;i<Nscenes;i++)
                    gist[i] = new double[Nfeatures];

                // Loop: Compute gist features for all scenes
                //for (int n = 0; n < Nscenes; n++)


                double[][] img = new double[param.ImageSize[0]][];
                for (int i = 0; i < param.ImageSize[0];i++ )
                    img[i] = new double[param.ImageSize[1]];

                // D is in gray scale

                //resize and crop image to make it square
                
                img = imresizecrop(D, param.ImageSize[0], param.ImageSize[1]);
                Size newsize = new Size(param.ImageSize[1], param.ImageSize[0]);  // size(width, height)
                ...
                gist = gistGabor(output, newsize, param, ref gistcontainer, numberOfImage);
                //gist(n,:) = g;
            ...
            catch (System.IO.FileNotFoundException)
            {
                ...
                System.Diagnostics.Debug.WriteLine("There was an error opening the image." + "Please check the path.");
            }
        ...
        public double[][] gistGabor(double[][] imgsrc, Size newsize, Parameter param, ref GistContainer gistcontainer, int numberOfImage)
        {
            //img = single(img);
            ...
            //int yshift = (int)Math.Floor(ny / 2.0);
            //int xshift = (int)Math.Floor(nx / 2.0);
            //circshift(ref img, nx, ny, yshift, xshift);
            ...
            if (numberOfImage==1)
            {
                gistcontainer.refImageRows = W * Nfilters;
                gistcontainer.refImageCols = N;

                gistcontainer.refImage = new double[gistcontainer.refImageRows][];
                for (int i = 0; i < gistcontainer.refImageRows; i++)
                {
                    gistcontainer.refImage[i] = new double[gistcontainer.refImageCols];
                }

                gistcontainer.refImage = g;
                

            }
            else 
            {
                gistcontainer.secondImageRows = W * Nfilters;
                gistcontainer.secondImageCols = N;

                gistcontainer.secondImage = new double[gistcontainer.secondImageRows][];
                for (int i = 0; i < gistcontainer.secondImageRows; i++)
                {
                    gistcontainer.secondImage[i] = new double[gistcontainer.secondImageCols];
                }

                gistcontainer.secondImage = g;
        ...
        public double FindDistance( GistContainer gistcontainer )
        {
            double sum = 0;
            for (int i = 0; i < gistcontainer.refImageRows; i++)
            {
                for (int j = 0; j < gistcontainer.refImageCols; j++)
                {
                    sum = sum + Math.Pow((gistcontainer.refImage[i][j] - gistcontainer.secondImage[i][j]), 2);
                }
            }

            gistcontainer.distance = sum;
            //PetrelLogger.InfoBox("Distance : " + sum);
            return gistcontainer.distance;
        ...
        public static void Main()
        {
            ...
            gist.LMgist("D:/Codes/Petrel 2015 Plugin/LMgistPlugin1/LMgistPlugin1/Resources/demo1.jpg", ref gistcontainer, 1);
            gist.LMgist("D:/Codes/Petrel 2015 Plugin/LMgistPlugin1/LMgistPlugin1/Resources/demo4.jpg", ref gistcontainer, 2);

            watch.Stop();
            var elapsedSec = watch.ElapsedMilliseconds / 1000.0;
            Console.Write("Distance14 : {0} elapsed time: {1}", gist.FindDistance( gistcontainer), elapsedSec);
            using (System.IO.StreamWriter file = new System.IO.StreamWriter(@"D:/Codes/Petrel 2015 Plugin/LMgistPlugin1/LMgistPlugin1/Resources/result.txt", true))
            {
                file.WriteLine("Distance14 : {0} elapsed time: {1}", gist.FindDistance( gistcontainer), elapsedSec);
            }
